   ...
   *     14.  Custom JS              *
   ==================================*/
    ...
    const tabs_func = function () {
        ...
        if (set_tabs != null) {
            function TabClicks(tabClickEvent) {
                ...
                const tabParent =
                    tabClickEvent.currentTarget.parentNode.parentNode;
                const set_tabs =
                    tabParent.querySelectorAll('[data-type="tabs"]');
                const tabIndicator =
                    tabParent.querySelectorAll('[role="indicator"]');
        ...
        if (range != null) {
            for (let i = 0; i < range.length; i++) {
                ...
                range[i].oninput = function () {
                    ...
                    range[i].nextElementSibling.style.width =
                        range[i].value + "%";
                };
            }
        }
    ...
    const snackbar_func = function () {
        const set_snackbars = document.querySelectorAll(
            '[data-type="snackbar"]'
        );
    ...
    const accordion_func = function () {
        const set_accordion = document.querySelectorAll(
            '[data-type="collapse"]'
        );

        if (set_accordion != null) {
            const is_accordion =
                document.querySelectorAll('[role="collapsed"]');
            // show
